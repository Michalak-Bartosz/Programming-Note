...
**NOTE:** Methods in an [Interface](../007_interface.md) that are not declared as default or static ([Default Methods in Interfaces](../../jdk/java-se-8-lts/features/language/001_default-methods-in-interfaces.md)) are implicitly abstract, so the abstract modifier is not used with interface methods (It can be used, but it is unnecessary).
