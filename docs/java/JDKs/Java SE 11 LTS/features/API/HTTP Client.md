...
We can do that in two ways — using the constructor for _Builder_ with _URI_ ([URI, URL and URN](uri,-url-and-urn.md)) parameter or calling method _uri(URI)_ on the _Builder_ instance:
```java
//constructor
...
With new API and builder, it’s straightforward to set a *CookieHandler* ([Cookie](cookie.md)) for our connection. We can use builder method *cookieHandler(CookieHandler cookieHandler)* to define client-specific *CookieHandler*.
...
The method *uri()* on the response object returns the *URI* [URI, URL and URN](uri,-url-and-urn.md) from which we received the response.
...
New *HttpClient* supports push promises through *PushPromiseHandler* interface [Interface](interface.md).
