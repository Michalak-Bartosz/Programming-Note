...
1. [Lambda Expressions](lambda-expressions.md)
# 3. <ins>Tips:</ins>
...
A compiler, in most cases, is able to resolve the type of lambda parameters with the help of [Type Inference](type-inference.md). Consequently, adding a type to the parameters is optional and can be omitted.
...
According to the [Effectively Final Variables](effectively-final-variables.md) concept, a compiler treats every variable as _final_ as long as it is assigned only once.
