...
#TODO
In Java, an _interface_ is an **abstract type** that contains a collection of methods and constant variables. It is one of the core concepts in Java and is used to **achieve [[Abstraction]], [[Polymorphism]] and [[Inheritance]]**.
* **Example:**
```java
...
```
All method declarations in an interface, including default methods, are implicitly public, so you can omit the public modifier. We see the interface has a default method. This feature was provided in Java SE 8 LTS version (More info: [[Default Methods in Interfaces]]).
...
* we **can’t** use the final word ([[Final Modifier]]) in the interface definition, as it will result in a compiler error
...
* interface variables **are public, static, and final by definition** ([[Java Modifiers]]); we’re **not allowed** to change their visibility
...
```
More: [[Comparator and Comparable]]
...
More: [[Abstract Classes Compared to Interfaces]]
