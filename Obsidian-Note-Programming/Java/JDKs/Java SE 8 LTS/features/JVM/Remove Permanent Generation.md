...
The JVM keeps track of loaded class metadata in the _PermGen_. Additionally, the JVM ([[Java Virtual Machine]]) stores all the static content in this memory section. This includes all the static methods, primitive variables, and references to the static objects.

Furthermore, **it contains data about bytecode, names, and JIT information** (Just-In-Time - [[JIT Compilation]]). Before Java 7, the String Pool was also part of this memory. The disadvantages of the fixed pool size are listed in [[Java String Pool]].
...
* **_MinMetaspaceFreeRatio_** – is the minimum percentage of class metadata capacity free after garbage collection ([[JVM Garbage Collectors]])
...
Despite all of these improvements, we still need to monitor and tune the metaspace to avoid memory leaks ([[JVM Parameters]]).
